FOCALIZE_SRC= \
  Finite_Set_S.fcl \
  Finite_Anti_set_S.fcl \
  Les_Fonctions_de_conversion.fcl \
  Un_Territoire_S.fcl \
  Le_Territoire_francais_S.fcl \
  Un_Texte_legislatif_S.fcl \
  Un_Organe_deliberant_S.fcl \
  Une_Loi_S.fcl \
  Le_Droit_francais_S.fcl \
  Une_Ordonnance_S.fcl \
  Un_Decret_S.fcl \
  Un_Etre_humain_S.fcl \
  Un_Francais_S.fcl \
  Le_Peuple_francais_S.fcl \
  Un_Haut_fonctionnaire_S.fcl \
  Un_Scrutin_S.fcl \
  Un_Scrutin_direct_S.fcl \
  Un_Scrutin_indirect_S.fcl \
  Un_Citoyen_electeur_S.fcl \
  L_Electorat_S.fcl \
  Une_Circonscription_S.fcl \
  Un_Elu_S.fcl \
  Un_Parlementaire_S.fcl \
  Le_Parlement_S.fcl \
  Un_Depute_S.fcl \
  Un_Senateur_S.fcl \
  Le_Senat_S.fcl \
  Un_Acte_de_l_executif_S.fcl \
  Un_Membre_de_l_executif_S.fcl \
  Un_Ministre_S.fcl \
  Un_Membre_du_gouvernement_S.fcl \
  Le_Premier_ministre_S.fcl \
  L_Executif_S.fcl \
  L_Assemblee_nationale_S.fcl \
  Le_Gouvernement_S.fcl \
  Le_Conseil_des_ministres_S.fcl \
  La_Constitution_S.fcl \
  La_Souverainete_nationale_S.fcl \
  Le_President_de_la_republique_S.fcl \
 
